...
{
if (caller == NULL || caller->mram == NULL || 
  caller->active_mswp == NULL ||
  vicfpn < 0 || swpfpn < 0) {
  return -1;
}
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz)
{
  ...
  cur_vma->vm_end = cur_vma->vm_end + inc_amt;  // Update vm_end to new boundary by a page-aligned size
